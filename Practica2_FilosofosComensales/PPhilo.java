...
public class PPhilo extends JPanel implements ActionListener {
    ...
    void simulate(String s) {
        ...
        case 1: 
            ...
            t = new RegularTable(sman);
            //Todos los filosofos-1 cogen el tenedor derecho
            for (i=0; i<4; i++){
                philo[i]=new Philo(i,cycles,delay,t); 
            }
            //El último filosofo coge el tenedor izquierdo
            philo[4]=new LefthandedPhilo(i,cycles,delay,t); 
            break;
        case 2: 
            System.out.println("Assimetry (even/odd).- TO BE IMPLEMENTED");
            t = new RegularTable(sman);
            //Si el filosofo es par coge el derecho
            //Si es impar se coge el tenedor izquierdo
            for (i=0; i<5; i++){
                if(i%2==0){
                    philo[i]=new Philo(i,cycles,delay,t); 
                }else{
